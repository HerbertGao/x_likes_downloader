...
通过 `xld serve --mcp` 暴露一个本地 stdio MCP server，把 likes 列举、媒体下载、凭据自检、cURL 导入与单条推文抓取等能力以 5 个 MCP 工具的形式提供给 Agent 调用，并保证凭据等敏感字段不经 MCP 通道泄露。
...
工具的 JSON Schema 必须由对应 Rust 请求类型自动派生（`schemars` crate），保证类型定义与 schema 一致。`fetch_tweet` 的输入类型 `FetchTweetRequest` 与现有 MCP 请求类型组织方式一致。
...
- 工具参数从请求的 `arguments` 字段解析；`download_media` 的 `items[]` 字段必须能反序列化为 `agent::types::MediaItem` 数组；`fetch_tweet` 的 `arguments` 必须能反序列化为 `FetchTweetRequest`（含互斥的 `url` 或 `id` 字段；schemars 无法表达「恰好其一」，互斥由 lib 层运行时校验兜底并由单测覆盖）
